---
title: "DEseq2"
author: "RuiLi, some parts modified by Kai Hu"
date: "03/02/2020"
output:
  html_document:
    toc: yes
...
# Read Data
```{r}
df <- read.table('counts.gene_id.s0.strict.txt', 
                 sep="\t", header=TRUE, 
...
```{r}
anno <- read.table(
"~/Desktop/bpipes/rnaseq/annotations/gencode.hu.v29/gencode.v29.primary_assembly.anno.txt.gz",
    header=T)
colnames(anno)
...
```{r design}
sample <- factor(rep(c("csc", "csc_diff", 'non_csc', 'non_csc_diff'), each=2))
sample
...
## One Big Model: non_csc vs csc
```{r}
#resultsNames(dds)
name <- "csc_diff_vs_csc"
contrast <- c('sample', 'csc_diff', 'csc')
